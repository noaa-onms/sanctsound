...
sounds <- get_modal_file_tbl("Sound", params$sanctuary_code, params$modal_title)

show_sites <- any(!is.na(sounds$site_id))
sight_sounds_md(sight, sounds, show_sites = show_sites)
```

```{r, results = "asis", eval=show_sites}
cat("\n### Map\n")
```

```{r, eval=show_sites}
map_site(
  tolower(params$sanctuary_code), 
  sites=na.omit(sounds$site_id))
```
...
if (nrow(monthly) == 1)
  ...
  <img src='{monthly$path_relative}' width='80%' class='mx-auto d-block'>
...
if (nrow(hourly) == 1)
  ...
  <img src='{hourly$path_relative}' width='80%' class='mx-auto d-block'>
