...
base_url <- 'https://sanctsound.ioos.us'
skip_drive_auth <- F
...
if (Sys.getenv("GITHUB_ACTIONS") == "") {
  ...
  google_sa_json <- "~/My Drive/private/nms4gargle-774a9e9ec703.json"
...
if (!skip_drive_auth) {
  message(
    "non-interactively authenticating to GoogleDrive with Google Service Account"
  )
...
figure <- function(
  ...
  if (length(img) == 2 && length(caption) == 1) {
    ...
    header = sapply(header, function(x) {
  ...
  if (length(img) == 2 && length(caption) == 2) {
    ...
    caption = sapply(caption, function(x) {
      markdown::markdownToHTML(text = x, fragment.only = T)
    })
    header = sapply(header, function(x) {
      markdown::markdownToHTML(text = x, fragment.only = T)
...
get_sheet <- function(sheet, redo = F) {
  ...
  if (!exists(sheet, envir = globalenv()) | redo) {
    ...
    d <- read_sheet(gsheet, sheet) %>%
      select(-starts_with("..."))
    msg <- glue(
      "get_sheet(): read_csv(URL), assign to variable '{sheet}'\n  URL: {get_sheet_csv(sheet)}",
      .trim = F
    )
...
get_nms_ply <- function(nms, dir_pfx = here("")) {
  ...
  if (!file.exists(nms_shp)) {
    ...
    nms_url <- glue::glue(
      "https://sanctuaries.noaa.gov/library/imast/{nms}_py2.zip"
    )
...
if (!file.exists(sites_csv)) {
  sites <- tibble::tribble(
    ...
    dplyr::mutate(
      ...
      geometry = purrr::map(sf, function(x) {
        sf::st_cast(sf::st_combine(x), "MULTIPOLYGON") %>% .[[1]]
      })
    ) %>%
    dplyr::select(-sf) %>%
    sf::st_sf(crs = 4326) %>%
...
get_modal_tab_content <- function(tab_name, modal_title = NULL, rmd = NULL) {
  ...
  imgs <- modals %>%
    ...
    mutate(
      ...
      path_w = glue(
        "{path_ext_remove(path_0)}_w{image_width_inches}in.{path_ext(path_0)}"
      ),
...
img_convert <- function(
  ...
  if (is.na(path_from) | is.na(path_to)) {
    return(NA)
  }
  ...
  if (file.exists(path_to) & !overwrite) {
    return(F)
  }
...
modal_title_to_html_path <- function(
  sanctuary_code,
  modal_title,
  pfx = here("./modals")
) {
...
render_modal <- function(
  ...
  modal_html,
  modal_id,
  rmd = here("./modals/_modal_template.Rmd")
) {
...
render_page <- function(rmd) {
  render(
    ...
    html_document(
      ...
      self_contained = F,
      lib_dir = here("./modals/modal_libs"),
      mathjax = NULL
    )
  )
}
...
audio_to_spectrogram_video <- function(path, path_mp4) {
  ...
  if (Sys.getenv("GITHUB_ACTIONS") != "") {
    message(
      "Skipping audio_to_spectrogram_video() using ffmpeg since GITHUB_ACTIONS environmental variable is not empty"
    )
    return(NA)
  }
  ...
  cmd <- glue(
    "source ~/.bash_profile; ffmpeg -y -i {path} -codec:a libmp3lame -qscale:a 2 {path_mp3}"
  )
  res <- system(cmd)
  ...
  if (!file.exists(sounds_csv) | redo) {
    tbl_sounds <- get_sheet("figures") %>%
      ...
      left_join(
        get_sheet("lookups") %>%
          select(
            icon_sound_subcategory = sound_subcategory,
            sound_category = sound_cat,
            sound_subcategory = sound_subcat
          ),
...
story_card <- function(
  ...
  if (!is.na(story_link)) {
    link_html <- glue(
      "<a href='{story_link}' target='_blank' class='stretched-link'></a>"
    )
  }
  title_html <- ""
  if (!is.na(title)) {
    ...
    font_size <- ifelse(
      !is.na(title_size),
      glue("style='font-size: {title_size}'"),
      ""
    ) # defaults to 1.3rem
    title_html <- glue(
      "
